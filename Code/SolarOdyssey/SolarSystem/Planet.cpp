...
Planet::Planet()
{
    ...
    myContinentParams.myNoiseStrength = 2.0f;
    myContinentParams.myNoisePosScale = 0.7f;
    myContinentParams.myNoiseMin = -1.0f;
...
void Planet::Render(float delta)
{
    ...
    //PxTransform transform = PxTransform(PxVec3(0.0f), PxQuat(myRotation.y, PxVec3(0,1,0)));
    //myRigidBody->myRigidbody->setGlobalPose(transform);
    //myRigidBody->myRigidbody->setAngularVelocity(PxVec3(0, 0.05f, 0));
...
void Planet::Generate()
{
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        myTerrainFaces[i].ConstructMesh(myNoise, *this);

        myTerrainFacesCollision[i] = TerrainFace(50, directions[i]);
        myTerrainFacesCollision[i].ConstructMesh(myNoise, *this);
...
void Planet::TerrainFace::ConstructMesh(const SimplexNoise& noise, const Planet& planet)
{
    ...
    for (int y = 0; y < myResolution; ++y)
    {
        for (int x = 0; x < myResolution; ++x)
        {
            ...
            Vec3 continentNoisePos = (pointOnUnitSphere + planet.myContinentParams.myNoisePosition) * planet.myContinentParams.myNoisePosScale;
            float continentShape = noise.Fractal(planet.myContinentParams.myOctaveCount, continentNoisePos.x, continentNoisePos.y, continentNoisePos.z) * planet.myContinentParams.myNoiseStrength;
            continentShape = Math::Clamp(continentShape, planet.myContinentParams.myNoiseMin, planet.myContinentParams.myNoiseMax);

            Vec3 elevationNoisePos = (pointOnUnitSphere + planet.myElevationParams.myNoisePosition) * planet.myElevationParams.myNoisePosScale;
            float elevation = noise.Fractal(planet.myElevationParams.myOctaveCount, elevationNoisePos.x, elevationNoisePos.y, elevationNoisePos.z) * planet.myElevationParams.myNoiseStrength;

            float height = planet.myPlanetSize * (1.0f + continentShape * 0.01f + (elevation * 0.01f * continentShape));

            myMeshDescription.myPositions[i] = pointOnUnitSphere * height;
